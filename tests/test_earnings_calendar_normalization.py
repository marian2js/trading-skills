...
def test_earnings_calendar_fixture_normalizes_to_expected_schema(repo_root):
    ...
    raw = load_json(repo_root / "skills/earnings-preview/fixtures/upcoming-week.json")
    expected = load_json(repo_root / "skills/earnings-preview/fixtures/normalized-events.json")

    actual = adapter.normalize(raw)
    assert actual == expected, "Earnings preview normalization should match the canonical fixture output"
